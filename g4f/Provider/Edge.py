...
class Edge(AsyncGeneratorProvider):
    url = "https://edgeservices.bing.com/chat"
    working = True
...
async def create_conversation(session: ClientSession, tone: str, image: str = None, proxy: str = None, headers: dict = {}) -> Conversation:
    url = f'https://edgeservices.bing.com/edgesvc/turing/conversation/create?bundleVersion={bundleVersion}'
    async with await session.get(url, proxy=proxy, headers=headers) as response:
        data = await response.json()

        conversationId = data.get('conversationId')
        clientId = data.get('clientId')
        conversationSignature = response.headers.get('X-Sydney-Encryptedconversationsignature')
        # print(data, conversationSignature)
...
async def list_conversations(session: ClientSession) -> list:
    url = "https://edgeservices.bing.com/turing/conversation/chats"
    ...
    async with ClientSession(
        ...
        try:
            async with session.ws_connect('wss://sydney.bing.com/sydney/ChatHub', autoping=False, params={'sec_access_token': conversation.conversationSignature}, proxy=proxy) as wss:
                ...
                await wss.receive(timeout=1000)
                ...
                gen_image = False
                while not final:
                    msg = await wss.receive(timeout=1000)
                        ...
                        if response.get('type') == 1 and response['arguments'][0].get('messages'):
                            ...
                            if (message['contentOrigin'] != 'Apology'):
                                if 'adaptiveCards' in message:
                                    ...
                                    gen_image = True
                                    yield "I'll try\n"
                                    # url = f"\nhttps://edgeservices.bing.com/images/create?q={query}"
